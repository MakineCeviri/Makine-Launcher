...
void TranslationService::startTranslation(const QString& gameId, const QString& gameName, const QString& installPath)
{
    ...
    if (!m_coreBridge) {
        setPhase(TranslationPhase::Error);
        setProgress(0, tr("Sistem hatası: Core bağlantısı yok"));
        emit translationError(gameId, tr("Core bağlantısı kurulamadı"));
        m_isActive.store(false, std::memory_order_relaxed);
        emit isActiveChanged();
        return;
    }
    ...
    setProgress(0, tr("Oyun motoru tespit ediliyor..."));
    ...
    if (m_activeEngine == "Unknown") {
        setPhase(TranslationPhase::Error);
        setProgress(0, tr("Oyun motoru tespit edilemedi"));
        emit translationError(gameId, tr("Desteklenmeyen oyun motoru"));
        m_isActive.store(false, std::memory_order_relaxed);
        emit isActiveChanged();
        return;
    }
    ...
    setProgress(1.0, tr("Motor tespit edildi: %1").arg(m_activeEngine));

    // Phase 2: Extracting strings
    setPhase(TranslationPhase::Extracting);
    setProgress(0, tr("Metinler çıkarılıyor..."));
...
void TranslationService::onExtractionCompleted(int count)
{
    ...
    if (count == 0) {
        setPhase(TranslationPhase::Error);
        setProgress(0, tr("Çevrilecek metin bulunamadı"));
        emit translationError(m_activeGameId, tr("Oyunda çevrilecek metin bulunamadı"));
        m_isActive.store(false, std::memory_order_relaxed);
        emit isActiveChanged();
        return;
    }
    ...
    setProgress(0, tr("Çeviriler eşleştiriliyor..."));
...
void TranslationService::runMatchingAndQA(int /*extractedCount*/)
{
    ...
    QMetaObject::invokeMethod(this, [this, passed, failed, avgScore, qaIssues,
        ...
        setProgress(0, tr("Çeviriler uygulanıyor..."));
...
void TranslationService::onPatchCompleted(int count)
{
    ...
    setProgress(1.0, tr("Çeviri tamamlandı! %1 metin uygulandı").arg(count));
...
void TranslationService::pauseTranslation()
{
    ...
    const QString pausedSuffix = tr(" (Duraklatıldı)");
    setProgress(m_progress, m_statusMessage + pausedSuffix);
    emit isPausedChanged();
}
...
void TranslationService::resumeTranslation()
{
    ...
    if (msg.endsWith(pausedSuffix)) {
        msg = msg.left(msg.length() - pausedSuffix.length());
