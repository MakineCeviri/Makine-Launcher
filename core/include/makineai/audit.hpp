/**
 * @file audit.hpp
 * @brief Audit logging and secure memory utilities for MakineAI
 *
 * Provides:
...
public:
    ...
    void log(AuditEvent event) {
        ...
        if (config_.logToConsole) {
            MAKINEAI_LOG_INFO("AUDIT", "{}", event.toLogString());
        }
...
private:
    AuditLogger() {
        // Default log path
        config_.logFile = fs::temp_directory_path() / "makineai" / "audit.log";
    }
