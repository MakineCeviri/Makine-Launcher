/**
 * @file lazy.hpp
 * @brief Lazy loading and deferred initialization utilities for MakineAI
 *
 * Provides:
...
public:
     ...
     */
    [[nodiscard]] Result<std::reference_wrapper<T>> tryGet() {
        ensureInitialized();
        if (error_) {
    ...
    [[nodiscard]] Result<std::reference_wrapper<const T>> tryGet() const {
        ensureInitialized();
        if (error_) {
            return std::unexpected(*error_);
        }
...
private:
    void ensureInitialized() const {
        ...
        try {
            ...
            const_cast<Lazy*>(this)->error_ = Error(ErrorCode::InitializationFailed,
...
public:
     ...
     */
    [[nodiscard]] Result<const std::string&> content() {
        return load();
    }
...
private:
    Result<const std::string&> load() {
        ...
        if (!file.read(content_.data(), fileSize)) {
            error_ = Error(ErrorCode::FileReadFailed,
        ...
        MAKINEAI_LOG_DEBUG("LAZY", "Loaded file: {} ({} bytes)", path_.string(), content_.size());
        return content_;
    }
...
public:
    ...
    [[nodiscard]] Result<T> tryGet(const std::string& path) {
        ...
        try {
            ...
            return std::unexpected(Error(ErrorCode::ParseFailed,
     ...
     */
    [[nodiscard]] Result<const Json&> json() {
        return parse();
    }
...
private:
    Result<const Json&> parse() {
        ...
        try {
            ...
            error_ = Error(ErrorCode::ParseFailed,
...
public:
    ...
    [[nodiscard]] Result<const T&> get() const {
        ensureInitialized();
        if (error_) {
            return std::unexpected(*error_);
        }
