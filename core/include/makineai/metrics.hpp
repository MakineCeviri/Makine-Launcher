...
/**
 * @file metrics.hpp
 * @brief Performance metrics collection for MakineAI
 *
 * Provides:
 ...
 */
#define MAKINEAI_METRIC_TIMER(name) \
    auto _metric_timer_##__LINE__ = ::makineai::metrics().timer(name)

/**
 * @brief Macro for timing a function
 */
#define MAKINEAI_METRIC_FUNCTION() \
    MAKINEAI_METRIC_TIMER(__FUNCTION__)
