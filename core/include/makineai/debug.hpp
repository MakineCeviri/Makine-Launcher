/**
 * @file debug.hpp
 * @brief Debug dump utilities for MakineAI
 *
 * Provides:
        ...
        if (dumpOnSuccess_ || !success_) {
            try {
                if (dumpFn_) {
                    ...
                    MAKINEAI_LOG_DEBUG("DEBUG", "Scope dump for {}: {} bytes", name_, content.size());
                }
            } catch (...) {
...
public:
    ...
    Result<fs::path> dumpState(const std::string& label, const std::string& content) {
        ...
        try {
            ...
            ofs << "=== MakineAI Debug Dump ===\n";
            ofs << "Label: " << label << "\n";
            ofs << "Timestamp: " << getCurrentTimestamp() << "\n";
            ofs << "================================\n\n";
            ofs << content;

            MAKINEAI_LOG_DEBUG("DEBUG", "Created dump: {}", path.string());
            cleanupOldDumps();
            return path;
    ...
    Result<fs::path> saveCrashReport(const CrashReport& report) {
        ...
        try {
            ...
            MAKINEAI_LOG_ERROR("DEBUG", "Crash report saved: {}", txtPath.string());
            return txtPath;
    ...
    [[nodiscard]] std::vector<fs::path> listDumps() const {
        ...
        for (const auto& entry : fs::directory_iterator(config_.dumpDirectory, ec)) {
            if (entry.is_regular_file() &&
                entry.path().filename().string().starts_with("makineai_")) {
    ...
    void clearDumps() {
        ...
        MAKINEAI_LOG_INFO("DEBUG", "Cleared all dump files");
    }

private:
    DebugDumper() {
        // Default dump directory
        config_.dumpDirectory = fs::temp_directory_path() / "makineai_dumps";
        std::error_code ec;
        fs::create_directories(config_.dumpDirectory, ec);
    }
