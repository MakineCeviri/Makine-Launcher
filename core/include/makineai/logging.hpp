/**
 * @file logging.hpp
 * @brief MakineAI structured logging system
 ...
 */
#define MAKINEAI_LOG_TRACE(module, fmt_str, ...) \
 ...
 */
#define MAKINEAI_LOG_DEBUG(module, fmt_str, ...) \
 ...
 */
#define MAKINEAI_LOG_INFO(module, fmt_str, ...) \
 ...
 */
#define MAKINEAI_LOG_WARN(module, fmt_str, ...) \
 ...
 */
#define MAKINEAI_LOG_ERROR(module, fmt_str, ...) \
 ...
 */
#define MAKINEAI_LOG_CRITICAL(module, fmt_str, ...) \
 ...
 * @example
 * void scanGames() {
 *     MAKINEAI_TIMED_SCOPE(log::SCANNER, "scan_all_games");
 *     // ... operation ...
 * }
 */
#define MAKINEAI_TIMED_SCOPE(module, operation) \
    ::makineai::ScopedTimer _timer_##__LINE__(operation, module)

/**
 * @brief Macro for creating a scoped timer at info level
 */
#define MAKINEAI_TIMED_SCOPE_INFO(module, operation) \
    ::makineai::ScopedTimer _timer_##__LINE__(operation, module, spdlog::level::info)
 ...
 */
#define MAKINEAI_LOG_IF(condition, level, module, ...) \
    do { \
        if (condition) { \
 ...
 */
#define MAKINEAI_LOG_ONCE(level, module, ...) \
    do { \
 ...
 */
#define MAKINEAI_LOG_THROTTLE(ms, level, module, ...) \
    do { \
 ...
 */
#define MAKINEAI_LOG_ENTER(module) \
    MAKINEAI_LOG_TRACE(module, "-> {}", __func__)

/**
 * @brief Log function exit (trace level)
 */
#define MAKINEAI_LOG_EXIT(module) \
    MAKINEAI_LOG_TRACE(module, "<- {}", __func__)
 ...
 */
#define MAKINEAI_LOG_FUNCTION(module) \
    ::makineai::ScopedFunctionLogger _funcLogger_##__LINE__(module)
 ...
 */
#define MAKINEAI_ASSERT(condition, module, ...) \
    do { \
        if (!(condition)) { \
            MAKINEAI_LOG_CRITICAL(module, "Assertion failed: " #condition); \
            MAKINEAI_LOG_CRITICAL(module, __VA_ARGS__); \
            std::abort(); \
        } \
    } while(0)
 ...
 */
#ifdef NDEBUG
    #define MAKINEAI_DEBUG_ASSERT(condition, module, ...) ((void)0)
#else
    #define MAKINEAI_DEBUG_ASSERT(condition, module, ...) \
        MAKINEAI_ASSERT(condition, module, __VA_ARGS__)
#endif
...
/**
 * @brief Configure logging for MakineAI
 *
