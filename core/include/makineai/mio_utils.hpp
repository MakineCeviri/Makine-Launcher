...
public:
    ...
    static Result<MappedFile> open(const fs::path& path) {
        ...
        mapped.path_ = path;
        MAKINEAI_LOG_TRACE(log::CORE, "Memory-mapped file: {} ({} bytes)",
...
public:
    ...
    static Result<MappedFile> open(const fs::path& path) {
        ...
        mapped.path_ = path;
        MAKINEAI_LOG_TRACE(log::CORE, "Loaded file into memory: {} ({} bytes)",
