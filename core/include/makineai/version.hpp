...
/**
 * @file version.hpp
 * @brief Version information and ABI compatibility for MakineAI
 *
 * Provides:
 ...
 * @code
 * // Get version string
 * std::cout << makineai::VERSION_STRING << std::endl;  // "0.1.0"
 *
 * // Check ABI compatibility
 * if (makineai::isABICompatible(library_abi_version)) {
 *     // Safe to use library
 * }
 *
 * // Get full build info
 * auto info = makineai::getBuildInfo();
 ...
 * Usage:
 * @code
 * #if MAKINEAI_VERSION_AT_LEAST(1, 0, 0)
 *     // Use new API
 * #else
 *     // Use old API
 * #endif
 * @endcode
 */
#define MAKINEAI_VERSION_AT_LEAST(major, minor, patch) \
    ((::makineai::VERSION_MAJOR > (major)) || \
     ((::makineai::VERSION_MAJOR == (major)) && (::makineai::VERSION_MINOR > (minor))) || \
     ((::makineai::VERSION_MAJOR == (major)) && (::makineai::VERSION_MINOR == (minor)) && (::makineai::VERSION_PATCH >= (patch))))
 ...
 */
#define MAKINEAI_VERSION_ENCODE(major, minor, patch) \
    ((major) * 10000 + (minor) * 100 + (patch))

/**
 * @brief Current version as encoded number
 */
#define MAKINEAI_VERSION_NUMBER MAKINEAI_VERSION_ENCODE(0, 1, 0)
