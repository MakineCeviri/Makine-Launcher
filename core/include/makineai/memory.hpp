...
/**
 * @file memory.hpp
 * @brief Memory profiling and tracking utilities for MakineAI
 *
 * Provides:
 ...
 */
#define MAKINEAI_TRACK_MEMORY(category) \
    ::makineai::ScopedMemoryTrack _memory_tracker_##__LINE__(category)

/**
 * @brief Record explicit allocation
 */
#define MAKINEAI_MEMORY_ALLOC(category, bytes) \
    ::makineai::MemoryTracker::record(category, bytes)

/**
 * @brief Record explicit release
 */
#define MAKINEAI_MEMORY_FREE(category, bytes) \
    ::makineai::MemoryTracker::release(category, bytes)
