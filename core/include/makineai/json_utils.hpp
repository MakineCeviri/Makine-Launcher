...
inline Result<JsonDocument> JsonDocument::parse(std::string_view jsonStr) {
    ...
    try {
        ...
        MAKINEAI_LOG_TRACE(log::CORE, "Parsed JSON with simdjson ({} bytes)", jsonStr.size());
        return doc;
    }
    catch (const std::exception& e) {
        // Fall through to nlohmann
        MAKINEAI_LOG_TRACE(log::CORE, "simdjson failed, falling back to nlohmann: {}", e.what());
    }
#endif
...
inline Result<JsonDocument> JsonDocument::parseFile(const fs::path& path) {
    ...
    try {
        ...
        MAKINEAI_LOG_TRACE(log::CORE, "Parsed JSON file with simdjson: {}", path.string());
        return doc;
    }
    catch (const std::exception& e) {
        MAKINEAI_LOG_TRACE(log::CORE, "simdjson failed for file, falling back: {}", e.what());
    }
#endif
